...
wordcloud2(demoFreq, size=1.6, color=rep_len( c("yellow","blue"), nrow(demoFreq) ) )
word = c('marketing','consumer','demand','price')
freq = c(15,26,15,36)
df1 = data.frame(word, freq)
rownames(df1) = word
head(df1)
wordcloud2(df1, size= .4, color = c('yellow','blue','green','pink'))
wordcloud2(df1, demoFreq, size = .4,shape = 'pentagon', backgroundColor = 'skyblue')

df = edit(df1)
# Change the background color
wordcloud2(df1, demoFreq, size=1.6, color='random-light', backgroundColor="white")

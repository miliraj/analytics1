...
#Each time we draw a sample of size n = 5 from the normal distribution having mean ?? = 100 and standard deviation ?? = 10, we need someplace to store the mean of the sample. Because we intend to collect the means of 500 samples, we initialize a vector xbar to initially contain 500 zeros
(xbar=rep(0,500))
mu=100; sigma=10 ; n=5
...
mean(xbar)
#distribution of 500 sample means, each of which was found by selecting n = 5 numbers from the normal distribution with mean ?? = 100 and standard deviation ?? = 10, then computing their mean (average)
